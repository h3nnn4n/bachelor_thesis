...
\chapter{Fundamentação Teórica} Neste capitulo são apresentados os conceitos
básicos e a fundamentação teórica necessária para o entendimento e abordagem ao
...
genérica de um problema de programação linear é:
...
Cada restrição em~\eqref{restricoes} pode ser convertida para restrição de de
...
e igualdades, sempre é possível reestrutura-lo através da adição de variáveis
...
\begin{align} \label{fo} \text{maximizar} \: z &= \sum_{j=1}^{n} c_jx_j
    \intertext{sujeito a:} \label{res} \sum_{j=1}^{n} a_{ij} x_j \leq b_i, i &=
    1, 2, \ldots, m \\ \label{res_t} x_j \geq 0, j &= 1, 2, \ldots, n
    \intertext{que é equivalente a:} \label{fo2} \text{maximizar} \: z &= cx
\intertext{sujeito a:} \label{res2} Ax &= b \\ \label{res_t2} x &\geq 0,
\end{align}
...
está e um vértice do poliedro, é necessário apenas explorar os vértices. Tendo
...
em tempo polinomial. No entanto, na prática o método possui um desempenho
inferior ao simplex e apresenta problemas de instabilidade numérica. O método
...
programação linear, denominado de método do ponto interior.  O método consiste
em a partir de uma solução factível centro do politopo, perturba-la até que a
mesma convirja para o ponto ótimo. O método do ponto interior também é
...
\begin{align} \label{} \text{maximizar} \: z &= cx \intertext{sujeito a:}
    \label{prnt} Ax &= b \\ \label{prt} x &\geq 0, \intertext{e} \label{}
    \text{minimizar} \: d &= ub \intertext{sujeito a:} \label{drnt} uA &= c \\
    \label{drt} u &\geq 0,
\end{align}
...
\begin{enumerate} \label{x} \item Se $\overline{x}$ satisfaz~\eqref{prnt}
    ...
    então $c\overline{x} \leq \overline{u}b$; \item Se $\overline{x}$ e
        $\overline{u}$ forem soluções factíveis do problema primal e dual,
        respectivamente, e $c\overline{x} = \overline{u}b$, então
        $\overline{x}$ é a solução ótima do problema primal e $\overline{u}$ é
    ...
    \item Se $\tilde{x}$ é a solução ótima do problema primal e $\tilde{u}$ é a
solução ótima do problema dual, então $c\tilde{x} = \tilde{u}b$.
\end{enumerate}

O primeiro item é conhecido como teorema da dualidade fraca, e sua implicação é
que uma solução dual é um limite superior de otimalidade para o problema
primal.  O segundo e terceiro item constituem o teorema da dualidade forte, que
...
problemas, visto que o valor fracionário das variáveis pode não ser condizente
com o que deseja-se resolver. Considerando um grafo, por exemplo, pode-se
modelar um problema onde $x_{ij} = 1$ se um vértice $i$ esta ligada a um
vértice $j$ e $0$ caso contrário.  Ao resolver o modelo, pode ocorrer que o
valor da variável venha a ser algo entre $0$ e $1$, que não possui
...
Outros problemas podem precisar de variáveis inteiras e fracionárias, estes
...
improvável que exista um algoritmo pseudo-polinomial para
resolve-lo~\cite{garey1978strong}.
...
\begin{figure}[!htb] \centering \begin{minipage}{.5\textwidth} \centering
    \begin{tabular}{r l} função objetivo &= $6.9090\ldots$ \\ $x_1$ &=
    $3.2727\ldots$ \\ $x_2$ &= $3.6363\ldots$ \end{tabular}
    \captionof{table}{Instância do CSP} \label{tab_csp} \end{minipage}%
    \begin{minipage}{0.5\textwidth} \centering
        \includegraphics[width=1\linewidth]{../figuras/pli.pdf} \caption{Espaço
        de busca do problema \eqref{pli_example}} \label{fig:name}
    \end{minipage} \end{figure}

Considerando que o problema de PLI admite apenas variáveis com valores
inteiros, o resultado acima é invalido como resposta para o problema.
...
O conceito de relaxação no contexto de otimização corresponde a remover algum
tipo de restrição. Um possível objetivo de relaxar um problema é facilitar a
sua solução.  Dentre as diversas restrições que podem ser relaxadas, uma delas
...
O espaço de busca de um problema de PLI consiste um om conjunto de pontos com
...
seja a mesma do problema original. Utilizando-se planos de corte pode-se
resolver problemas de PLI apenas com o SIMPLEX. O método funciona teoricamente,
porém na pratica o método sofre de sérios problemas de instabilidade numérica,
...
Anos depois, baseados nos trabalhos de~\cite{little1963algorithm},
~\cite{land2010automatic} propos um método de enumeração implicita que veio a
...
a melhores resultados e elimina regiões infrutiferas. Um tipico exemplo de
...
$z = 22.4$ e uma região possui limite superior de $z = 21.0$, esta região pode
ser descartada desde que seja encontrado uma solução factivel em $S_1$. Quanto
menor a folga entre a solução ótima e o limite superior e inferior, melhor o
BnB opera.
...
Na programação linear inteira existem problemas grandes demais para serem
...
problema $\mathcal{NP}$-difícil. Esta restrição insere uma quantidade
fatorial de restrições no modelo do TSP. Em~\eqref{fat_tsp} temos uma formula
fechada que descreve o número de restrições que~\eqref{stsp3} insere. Portanto
é necessário que se ultilize um procedimento denomidado de geração de linhas
...
a solução ótima $\tilde{x}^*$ não necessáriamente ira satisfazer
...
em~\cite{nemhauser1988integer}. Apesar da dificuldade do problema da separação,
...
O problema da separação é formulado como outro problema de PLI, formulado de
...
enquanto que o número de váriaveis é muito maior. Um exemplo de problema com
este comportamento é o \textit{Cutting stock problem}. O problema consiste
em dado uma demana de peças de tamanhos arbitrarios e um estoque de peças de
tamanho fixo, determinar como cortar o estoque para obter as peças demandadas
com um disperdicio mínimo. Sua formulação é dada a seguir:
...
número de restrições, em um problema onde exite uma quantidade muito maior
de colunas do que linhas boa parte do tempo de solução seria gasto processnado
