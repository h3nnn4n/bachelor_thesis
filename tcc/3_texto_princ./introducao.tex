...
A primeira etapa consiste em determinar quais locais devem ser atendidos pelo serviço de transporte e determinar qual
a melhor rota para cobrir estes locais. Deve ainda escolher a melhor localização para as estações de ônibus ao longo
do percurso do veículo. A segunda etapa consiste em baseado na demanda pelo serviço, determinar os horários na qual os
ônibus devem partir dos terminais, de modo a atender da forma mais eficiente possível. A terceira etapa possui a
funcionalidade de determinar quais veículos saem de quais estações, por quais pontos ele deve passar e para onde que
ele deve retornar, formando assim uma jornada. Esta etapa deve cobrir todas as tarefas e minimizar o custo operacional
A quarta e ultima etapa deve associa motoristas com jornadas, de modo que cada jornada tenha um motorista, onde
cada motorista tenha todos os seus direitos respeitados, tanto os previstos por lei quanto os contratuais.
...
de escolha está sujeito à restrições. Tais como regras definidas pela empresa; leis trabalhistas; sindicais
...
diversas heurísticas duais, para assim encontrar o limite superior de otimalidade do problema. Trabalhos como~\cite{de2011algoritmo} e
\cite{dos2008metodo} utilizam soluções híbridas para a solução do problema.\cite{ceria1997}, \cite{caprara2000algorithms},
...
O método de geração de colunas consiste em um processo o problema com um grande número de variáveis é decomposto em
...
soluções de modo exato, e preferencialmente mais eficiente, considerando algoritmos pesquisados na literatura. O algoritmo proposto será aplicado
a instâncias disponíveis na literatura.

No capitulo 2 é apresentado a fundamentação teoria necessária para o estudo do algoritmo proposto. Define-se programação linear, programação linear
inteira e é apresentado os seus respetivos métodos de solução.

No capitulo 3 é apresentado a formulação do CSP e sua respectiva modelagem utilizando-se geração de colunas, onde é definido também o problema mestre
e o subproblema.
